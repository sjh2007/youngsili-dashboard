...
export default function UpgradeModal(props: any) {
  const {
    ...
    payTestEnabled,
    startSubscription, paymentHistoryLoading, paymentHistory, setRefundTarget,
              ...
              <table style={{width:'100%',borderCollapse:'collapse',fontSize:14}}>
                ...
                <tbody>{paymentHistory.map((p:any)=>{
                  ...
                  <tr key={p.id} style={{borderBottom:'1px solid #f1f5f9'}}>
                    ...
                    <td style={{padding:'10px'}}>{p.type==='subscription' ? `정액제${p.planKey?`(${p.planKey})`:''}` : p.type==='trial' ? '기존 시범 이용 결제' : '크레딧 충전'}</td>
