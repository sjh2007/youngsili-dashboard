          ...
          <section className="subscription-overview" aria-labelledby="current-subscription-title">
            <div className="subscription-overview__header" style={{display:'flex',justifyContent:'space-between',gap:16,alignItems:'flex-start',flexWrap:'wrap'}}>
              <div>
                ...
                <div id="current-subscription-title" style={{fontSize:24,fontWeight:900,color:'#0f172a',marginTop:5}}>{billingPlanName(subStatus.plan)}</div>
            ...
            <div className="subscription-facts">
              <div className="subscription-fact"><div className="subscription-fact__label">결제수단</div><div className="subscription-fact__value">{subStatus.paymentMethod?.label||'등록된 결제수단'}</div>{subStatus.paymentMethod?.masked&&<div className="subscription-fact__meta">{subStatus.paymentMethod.masked}</div>}</div>
              <div className="subscription-fact"><div className="subscription-fact__label">다음 결제일</div><div className="subscription-fact__value">{subStatus.nextChargeAt?new Date(subStatus.nextChargeAt).toLocaleDateString('ko-KR'):'확인 필요'}</div></div>
              <div className="subscription-fact"><div className="subscription-fact__label">결제 상태</div><div className={`subscription-fact__value ${subStatus.lastChargeError?'is-error':'is-ok'}`}>{subStatus.lastChargeError?'최근 결제 실패':'정상'}</div></div>
            </div>
