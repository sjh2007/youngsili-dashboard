    ...
    <div className="modal-overlay" onClick={()=>setShowUpgradeModal(false)}>
      <div className="modal" onClick={e=>e.stopPropagation()} style={{maxWidth:920,width:'96%',textAlign:'left',maxHeight:'90vh',overflowY:'auto'}}>
        ...
        {currentSub?.autoRenew && upgradeTab!=='history' ? (<>
          ...
          {manageAction==='payment'&&<div className="subscription-editor"><b style={{fontSize:16}}>새 결제수단 등록</b><p style={{fontSize:13,color:'#64748b',lineHeight:1.6}}>새 수단의 빌링키가 정상 확인된 뒤 이 통화 방식의 기존 수단을 교체합니다. 지금 추가 결제되지는 않습니다.</p><div style={{display:'flex',gap:8,margin:'12px 0'}}>{(payTestEnabled?[['CARD','신용·체크카드'],['TRANSFER','계좌 자동이체']]:[['CARD','신용·체크카드']]).map(([value,label])=><button key={value} type="button" aria-pressed={subscriptionPaymentMethod===value} onClick={()=>setSubscriptionPaymentMethod(value)} style={{padding:'9px 14px',borderRadius:9,border:subscriptionPaymentMethod===value?'2px solid #246BEB':'1px solid #dbe4f0',background:subscriptionPaymentMethod===value?'#eaf2ff':'#fff',fontWeight:750}}>{label}</button>)}</div><button className="btn-primary" disabled={!!subscribeBusy} onClick={()=>startSubscription(currentSub.plan,billingPlanName(currentSub.plan),{replacePaymentMethod:true})}>{subscribeBusy?'등록 중...':'이 결제수단으로 변경'}</button></div>}

          {manageAction==='plan'&&<div style={{marginTop:14,padding:18,border:'1px solid #dbe4f0',borderRadius:14}}><b style={{fontSize:16}}>다음 결제일부터 적용할 요금제</b><p style={{fontSize:13,color:'#64748b'}}>현재 이용기간과 포함 통화량은 유지되며 다음 자동결제일부터 변경됩니다.</p>{upgradeTab==='metered'?<div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(190px,1fr))',gap:10,marginTop:12}}>{PSTN_SUBSCRIPTION_PLANS.map(plan=><div key={plan.key} style={{padding:15,border:'1px solid #e2e8f0',borderRadius:12}}><b>{plan.name}</b><div style={{fontSize:18,fontWeight:900,margin:'7px 0'}}>{plan.chargedAmount.toLocaleString()}원/월</div><div style={{fontSize:12,color:'#64748b'}}>포함 {plan.includedCalls}통 · {plan.channels}채널</div><button className="btn-primary" style={{width:'100%',marginTop:12}} disabled={subscriptionActionBusy==='plan'||(currentSub.plan===plan.key&&!currentSub.pendingPlan)} onClick={()=>scheduleSubscriptionPlanChange(plan.key,`일반전화 ${plan.name}`)}>{currentSub.pendingPlan===plan.key?'변경 예약됨':currentSub.plan===plan.key?'현재 요금제':'이 요금제로 변경'}</button></div>)}</div>:<div style={{marginTop:12}}><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(86px,1fr))',gap:8}}>{APP_PHONE_PLANS.map(plan=><button key={plan.key} onClick={()=>setSelectedAppKey(plan.key)} style={{padding:10,borderRadius:9,border:selectedAppKey===plan.key?'2px solid #246BEB':'1px solid #dbe4f0',background:selectedAppKey===plan.key?'#eff6ff':'#fff'}}>{plan.elderLimit}명</button>)}</div><div style={{display:'flex',gap:8,marginTop:10}}>{APP_WEEKLY_FREQUENCIES.map(f=><button key={f} onClick={()=>setSelectedAppFrequency(f)} style={{flex:1,padding:10,borderRadius:9,border:selectedAppFrequency===f?'2px solid #246BEB':'1px solid #dbe4f0',background:selectedAppFrequency===f?'#eff6ff':'#fff'}}>주 {f}회</button>)}</div><div style={{marginTop:12,fontWeight:850}}>월 {selectedAppPrice.toLocaleString()}원</div><button className="btn-primary" style={{width:'100%',marginTop:12}} disabled={subscriptionActionBusy==='plan'} onClick={()=>scheduleSubscriptionPlanChange(selectedAppPlanKey,`앱 ${selectedAppPlan.elderLimit}명·주 ${selectedAppFrequency}회`)}>이 요금제로 변경</button></div>}</div>}
        </>) : upgradeTab==='metered' ? (<>
          ...
          <p style={{color:'#64748b',fontSize:15,margin:'0 0 20px',lineHeight:1.6}}><b style={{color:'#0f172a'}}>앱 전화 안부콜</b>은 등록 인원과 주간 통화 횟수에 따라 월 요금이 정해집니다. 부가세 포함 금액이며 월 포함량을 초과하면 보유 크레딧에서 차감됩니다.</p>
          {payTestEnabled && <div style={{padding:'14px 16px',marginBottom:16,border:'1px solid #dbe4f0',borderRadius:12,background:'#f8fafc'}}><div style={{fontSize:13,fontWeight:800,color:'#0f172a',marginBottom:9}}>정기결제 수단</div><div style={{display:'flex',gap:8}}>{[['CARD','신용·체크카드'],['TRANSFER','계좌 자동이체']].map(([value,label])=><button key={value} type="button" aria-pressed={subscriptionPaymentMethod===value} onClick={()=>setSubscriptionPaymentMethod(value)} style={{padding:'9px 14px',borderRadius:9,border:subscriptionPaymentMethod===value?'2px solid #246BEB':'1px solid #dbe4f0',background:subscriptionPaymentMethod===value?'#eaf2ff':'#fff',color:subscriptionPaymentMethod===value?'#1d4ed8':'#475569',fontWeight:750,cursor:'pointer'}}>{label}</button>)}</div></div>}
          <div style={{border:'1px solid #dbe4f0',borderRadius:16,padding:18,background:'linear-gradient(145deg,#fff 0%,#f7faff 100%)'}}>
            ...
            <div style={{display:'flex',gap:10,marginTop:18,flexWrap:'wrap'}}><button className="btn-primary" style={{flex:1}} disabled={subscribeBusy===selectedAppPlanKey || currentSub?.plan===selectedAppPlanKey} onClick={()=>startSubscription(selectedAppPlanKey,`앱 ${selectedAppPlan.elderLimit}명·주 ${selectedAppFrequency}회`)}>{subscribeBusy===selectedAppPlanKey?'처리 중...':currentSub?.plan===selectedAppPlanKey?'자동결제 중':'이 요금제로 신청'}</button><button className="btn-secondary" disabled={!!billing?.trialEndsAt} onClick={startTrial}>{billing?.trialEndsAt?'이미 체험함':'30일 무료 체험'}</button></div>
          </div>
